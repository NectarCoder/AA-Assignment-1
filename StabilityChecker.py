...
class StabilityOutput:
    ...
    def isEveryOneInAggrement(self, positions: list, applicants: list)-> list:
        ...
        for currentApplicantName in self._matches:
            ...
            indexOfCurrentApplicantInPositionWishList = next(i for i, item in enumerate(currentPosition.getPreferences()) if item == currentApplicantName)
            ...
            for otherPosition in positions:
                if otherPosition.getName() != self._positionName: 
                    indexOfTheOtherPositionsApplicant = next(i for i, item in enumerate(otherPosition.getPreferences()) if item == currentApplicantName)
                    ...
                    if indexOfCurrentApplicantInPositionWishList > indexOfTheOtherPositionsApplicant:
                        ...
                        applicantInQuestion = next((applicant for applicant in applicants if applicant.getName() == currentApplicantName))
                        indexOfCurrentApplicantsPosition = next(i for i, item in enumerate(applicantInQuestion.getPreferences()) if item == self._positionName)
                        indexOfTheOtherApplicantsPosition = next(i for i, item in enumerate(applicantInQuestion.getPreferences()) if item == otherPosition.getName())
                        if indexOfCurrentApplicantsPosition > indexOfTheOtherApplicantsPosition:
                            ...
                            unstableMatches.append(f"Type 2 Instability: For, M={{{currentPosition.getName()},{','.join(self._matches)}}}. {otherPosition.getName()} prefers {currentApplicantName} over {currentPosition.getName()}, and {currentApplicantName} prefers {otherPosition.getName()} over the currently matched {currentPosition.getName()}")
